...
import { getUserWiseLeadStatus } from '../../../Services/analytics/leadStatusAnalyticsService';

const UserWiseLeadStatusTable = ({ contextParams = {}, activeStatusFilter = null }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const res = await getUserWiseLeadStatus(params);
                if (isMounted && res) {
                    setStatuses(res.statuses || []);
                    setUsers(res.users?.content || []);
                    setTotalElements(res.users?.totalElements || 0);
                    setTotalPages(res.users?.totalPages || 0);
                    setGrandTotals(res.grandTotalStatusCounts || {});
                    setTotalLeads(res.grandTotal || 0);
                }
            } catch (err) {
                console.error('Failed to load user-wise lead status data', err);
                if (isMounted) {
                    setUsers([]);
                    setTotalElements(0);
                    setTotalPages(0);
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p className="text-[11px] text-gray-500">
                            Distribution of allotted leads across team members and lead statuses
                        </p>
                    </div>
                    <span className="text-xs font-semibold bg-purple-50 text-purple-700 px-2 py-0.5 rounded-full border border-purple-100 ml-2">
                        {totalElements} Users
                    </span>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="pl-8 pr-3 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-purple-500 w-44 sm:w-56"
                        />
                    </div>
                </div>
            </div>
                    ...
                    <thead className="bg-gray-50 text-gray-600 uppercase text-[11px] font-semibold sticky top-0 z-10 shadow-sm border-b border-gray-200">
                        <tr>
                            ...
                            <th
                                ...
                                className="px-4 py-3 cursor-pointer hover:bg-gray-100 transition-colors whitespace-nowrap min-w-[190px] bg-gray-50 sticky left-0 z-20 border-r border-gray-200"
                            >
                                <div className="flex items-center gap-1.5">
                                    <span>User</span>
                                    {sortBy === 'userName' && (
                                        sortDirection === 'ASC' ? <FiArrowUp size={11} className="text-purple-600" /> : <FiArrowDown size={11} className="text-purple-600" />
                                    )}
                                </div>
                            </th>
                                ...
                                return (
                                    <th
                                        ...
                                        className={`px-3 py-3 text-center cursor-pointer hover:bg-gray-100 transition-colors whitespace-nowrap min-w-[110px] ${
                                            isHighlighted ? 'bg-purple-50/80 text-purple-700' : ''
                                        }`}
                        ...
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan={statuses.length + 2} className="py-10 text-center text-gray-400">
                                    No users found with allotted leads matching the selected criteria.
                                </td>
                            </tr>
                        ) : (
                            users.map((u, idx) => (
                                <tr key={u.userId || idx} className="hover:bg-purple-50/20 transition-colors">
                                    ...
                                    {statuses.map((st) => {
                                        const count = u.statusCounts?.[st.code] ?? 0;
                                        const isHighlighted = activeStatusFilter && (
                                            activeStatusFilter === st.code ||
                                            activeStatusFilter === st.statusId ||
                                            activeStatusFilter === st.name
                                        ...
                                        return (
                                            <td
                                                ...
                                                } ${isHighlighted ? 'bg-purple-50/40 font-bold text-purple-700' : ''}`}
                                            >
                                                {count}
                                            </td>
                                        );
                                    })}
                                    <td className="px-4 py-2.5 text-right font-mono font-bold text-purple-700 bg-gray-50/50 border-l border-gray-100">
                                        {u.total ?? 0}
                                    </td>
                                </tr>
                            ))
                    ...
                    {!loading && users.length > 0 && (
                        <tfoot className="bg-gray-100/90 text-gray-900 font-bold uppercase text-[11px] sticky bottom-0 z-10 border-t-2 border-gray-300 shadow-inner">
                            <tr>
                                    ...
                                    return (
                                        <td
                                            ...
                                            className={`px-3 py-3 text-center font-mono ${
                                                isHighlighted ? 'bg-purple-100 text-purple-800' : ''
                                            }`}
                                        >
                                            {total}
                                        </td>
                                ...
                                <td className="px-4 py-3 text-right font-mono text-purple-700 font-extrabold border-l border-gray-200 text-sm">
                                    {totalLeads}
                                </td>
                            </tr>
                        </tfoot>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-4 pt-3 border-t border-gray-100 text-xs text-gray-500">
                <div className="flex items-center gap-2">
                    ...
                    <select
                        ...
                        className="px-2 py-1 border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-purple-500 bg-white"
                    >
                    ...
                    <span className="text-gray-400">
                        Showing {users.length > 0 ? page * size + 1 : 0} - {Math.min((page + 1) * size, totalElements)} of {totalElements}
                    </span>
                </div>
...
export default UserWiseLeadStatusTable;
